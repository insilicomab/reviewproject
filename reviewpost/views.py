...
def signupview(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            return render(request, 'reviewpost/signup.html', {'error': 'このユーザーは既に登録されています'})
    else:
...
def loginview(request):
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            return redirect('reviewpost:login')
    return render(request, 'reviewpost/login.html')

@login_required
def listview(request):
    object_list = ReviewModel.objects.all()
    return render(request, 'reviewpost/list.html', {'object_list':object_list})

def detailview(request, pk):
    object = ReviewModel.objects.get(pk=pk)
    return render(request, 'reviewpost/detail.html', {'object':object})

class CreateClass(CreateView):
    template_name = 'reviewpost/create.html'
    model = ReviewModel
    fields = ('title', 'content','author', 'images', 'evaluation')
    success_url = reverse_lazy('reviewpost:list')
